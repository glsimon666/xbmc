  ...
  while (!m_bAbortRequest)
  {
    ...
    if (!pPacket)
    {
      ...
      if(next == CDVDInputStream::NEXTSTREAM_OPEN)
      {
        ...
        CLog::Log(LOGDEBUG, "VideoPlayer: next stream, wait for old streams to be finished");
        CloseStream(m_CurrentAudio, true);
        CloseStream(m_CurrentVideo, true);
      ...
      if (!m_pInputStream->IsEOF())
        CLog::Log(LOGDEBUG, "{} - eof reading from demuxer", __FUNCTION__);
...
void CVideoPlayer::SetUserDialogueSubtitleOffset(int offset)
{
  m_renderManager.SetUserDialogueSubtitleOffset(offset);
}
...
int CVideoPlayer::OnDiscNavResult(void* pData, int iMessage)
{
  ...
  if (m_pInputStream->IsStreamType(DVDSTREAM_TYPE_BLURAY))
  {
    ...
    case BD_EVENT_DISCONTINUITY:
      CLog::Log(LOGDEBUG,
                "CVideoPlayer::OnDiscNavResult - handling discontinuity");
      // Restore normal discontinuity handling
      m_messenger.Put(std::make_shared<CDVDMsg>(CDVDMsg::GENERAL_FLUSH));
      break;
    default:
      break;
